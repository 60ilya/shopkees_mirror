...
# The default config is an site mirror to `www.redsea.com` along with `*.redsea.com`
#   you can just copy this file to 'config.py' and then execute `python3 wsgi.py`,
#   it will start an localhost web server.
# Then, enter http://127.0.0.1/ you will actually access to the www.redsea.com  (the linux kernel website)
...
#
# 默认配置文件是对 www.redsea.com (linux内核的网站)及其所有子站的镜像.
#   请复制一份本文件为 'config.py' 然后运行 `python3 wsgi.py`
#   然后访问 http://127.0.0.1 , 你将看到的是 www.redsea.com 的首页
#   www.redsea.com 和它的所有子站都被自动地加入到了这个反向代理镜像中.
#   你可以在网站中随意点击, 随意浏览, 而不会跑到镜像外. (*.redsea.com以外的网站仍然会跑到外面, 因为把它们没有加入镜像)
#
...
external_domains = (
    # actually, the redsea.com has many sub-domains, but we just add one of them for example
    # the following `Automatic Domains Whitelist` would detect and add the others automatically
    #
    # 实际上, redsea.com 有大量的子域名, 但是我们在这里并不把它们全部添加进来, 而只是添加一个作为示例
...
domains_whitelist_auto_add_glob_list = ('*.redsea.com',)
...
#   eg: redsea.com is the same of www.redsea.com
#       format: ('redsea.com',)
